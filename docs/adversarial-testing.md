...
JSONC is the lossless format. A suite is `Suite → Cases → ordered Turns`; profiles link workspace-relative JSONC or CSV files with `tests.adversarialSuites`. Test Explorer displays either source as Profile → Suite → Case → Turn. A suite has no case-count or aggregate-turn ceiling; each case may contain up to 10 turns.
