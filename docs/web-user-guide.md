...
伺服器 `profiles/` 下的巢狀目錄會顯示在「預設」清單，由部署人員在伺服器上調整。個人的資料夾可按左側標題旁的資料夾圖示新增；資料夾旁可新增子資料夾、上移或下移、重新命名及刪除。選擇設定檔後，可用側欄下方的「資料夾」選單歸類。刪除個人資料夾前會要求確認，裡面的設定檔與子資料夾會移到上一層，不會刪除設定檔。搜尋時會顯示符合條件的設定檔，即使所屬資料夾原本收合。個人資料夾和歸類只儲存在目前瀏覽器，不會修改伺服器上的預設設定檔，也不包含在設定檔可攜匯出檔中。

## 建立與修改 Profile

### 檢視 JSONC 與設定檔指南

從左側設定檔的「⋯」選「檢視 JSONC」，即可在網頁內閱讀該筆設定檔的完整原始內容，包括註解；這個畫面是唯讀，不會修改設定檔。可搜尋文字、跳到設定區段、切換自動換行，也可複製內容或下載單一 `.turnstage.jsonc` 檔案。右側「設定」也可開啟目前選用設定檔的 JSONC。若要連同連線環境一起分享，仍應使用左側「匯出」產生的可攜檔。
...
設定檔列表中的「⋯」可直接對該筆設定檔檢視 JSONC、複製、匯出，或刪除本機設定檔。本機設定檔建立了自訂資料夾後，選單還會出現「移至資料夾」；預設設定檔依伺服器資料夾顯示，不能移入本機資料夾或刪除。使用鍵盤時，聚焦該列即可看到操作鈕，按 Escape 可關閉選單。
