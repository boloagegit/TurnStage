  ...
  return <div className="jsonc-reader">
    ...
    <div className="jsonc-reader-body">
      ...
      <div ref={codeRef} className={`jsonc-code-scroll${wrap ? ' is-wrapped' : ''}${onEdit ? ' is-editable' : ''}`} tabIndex={onEdit ? -1 : 0} aria-label={onEdit ? undefined : 'JSONC'}>
        {!onEdit && <div className="jsonc-lines">{renderedLines.map((segments, index) => <div className={`jsonc-line${matches[matchIndex] === index + 1 ? ' is-current-match' : ''}`} data-line={index + 1} key={index}><span className="jsonc-line-number" aria-hidden="true">{index + 1}</span><code>{highlight(segments, query)}</code></div>)}</div>}
        {onEdit && <textarea ref={activeEditorRef} className="jsonc-edit-textarea" aria-label="JSONC" spellCheck={false} wrap={wrap ? 'soft' : 'off'} value={raw} onChange={(event) => onEdit(event.target.value)} />}
      </div>
    </div>
  </div>;
}
