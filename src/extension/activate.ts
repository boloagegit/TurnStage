...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  const refreshProfileState = async () => { tree.refresh(); await Promise.all([duplicateDiagnostics.refresh(), scenarioTests.refresh()]); };
  ...
  command('createProfile', async (scopeItem?: ProfileScopeTreeItem) => { if (!requireWorkspaceTrust()) return; const destination = await pickProfileDestination(scopeItem?.scope); if (!destination) return; const uri = await createEmptyProfile(repository, destination); if (uri) { await refreshProfileState(); await vscode.commands.executeCommand('vscode.openWith', uri, 'turnstage.profileEditor'); void showNotification('information', vscode.l10n.t('Created empty profile {path}.', { path: displayProfilePath(uri, repository) })); } });
  ...
  command('replayRun', async (item?: ProfileTreeItem | vscode.Uri) => { const uri = asUri(item) ?? activeCustomEditorUri(); if (!uri) { void showNotification('error', vscode.l10n.t('Open a profile in the TurnStage editor first.')); return; } const controller = await openAndWaitForController(editor, uri, true); if (!controller) { void showNotification('error', vscode.l10n.t('The TurnStage profile editor did not become ready in time.')); return; } const result = await editor.replayRun(uri); if (result === 'started') return result; await editor.showSection(uri, 'test'); if (result === 'active') void showNotification('information', vscode.l10n.t('Finish or stop the current request before replaying another run.')); else if (result === 'unavailable') void showNotification('information', vscode.l10n.t('This run cannot be replayed because raw events were not recorded.')); else void showNotification('information', vscode.l10n.t('No recorded runs are available. Open Test to run a profile first.')); return result; });
  ...
  command('exportTestReport', async () => {
    ...
    const format = await vscode.window.showQuickPick([
      ...
      { label: vscode.l10n.t('JUnit XML'), value: 'junit' as const },
      { label: 'HTML', value: 'html' as const },
    ], { title: vscode.l10n.t('Export Conversation Contract Report') });
    if (!format) return;
    const uri = await scenarioTests.exportLastReport(format.value);
    if (uri) void showNotification('information', vscode.l10n.t('Contract report exported to {path}.', { path: displayExportUri(uri) }));
  });
  command('exportEvidenceBundle', async () => {
    if (!requireWorkspaceTrust()) return;
    if (!scenarioTests.hasReport()) { void showNotification('information', vscode.l10n.t('No conversation contract results are available. Run a scenario from Test Explorer first.')); return; }
    const uri = await scenarioTests.exportEvidenceBundle();
    if (uri) {
      const index = vscode.Uri.joinPath(uri, 'index.html');
      void showNotification('information', vscode.l10n.t('Evidence bundle exported to {path}.', { path: displayExportUri(uri) }));
  ...
  command('openTestEvidence', async (argument?: unknown) => {
    ...
    const evidenceId = typeof value?.evidenceId === 'string' ? value.evidenceId : undefined;
...
function isScenarioEvidenceLocation(value: unknown): value is ScenarioEvidenceLocation {
  if (!isRecord(value) || !['network', 'rawEvent', 'normalizedEvent', 'message', 'profile'].includes(String(value.kind))) return false;
  if (value.networkId !== undefined && (typeof value.networkId !== 'string' || value.networkId.length > 512)) return false;
  if (value.messageId !== undefined && (typeof value.messageId !== 'string' || value.messageId.length > 512)) return false;
  if (value.path !== undefined && (typeof value.path !== 'string' || value.path.length > 512)) return false;
  return ['sequence', 'rawSequence'].every((key) => value[key] === undefined || (Number.isInteger(value[key]) && Number(value[key]) >= 0));
}

function logScenarioTestError(output: vscode.OutputChannel, error: unknown): void { output.appendLine(`[error] [tests] ${error instanceof Error ? error.message : String(error)}`); }
...
async function openAndWaitForController(editor: TurnStageEditorProvider, uri: vscode.Uri, suppressAutoStart = false): Promise<import('./runtime/sessionController').SessionController | undefined> {
  if (suppressAutoStart && !editor.getController(uri)) editor.suppressNextAutoStart(uri);
  try {
    await openProfileSection(editor, uri, 'test');
    return await editor.waitForController(uri);
  } finally { if (suppressAutoStart) editor.clearAutoStartSuppression(uri); }
}
