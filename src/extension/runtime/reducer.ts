...
export function reduceEvent(snapshot: SessionSnapshot, event: NormalizedEvent): void {
  ...
  switch (event.type) {
    ...
    case 'tool.started': { const toolCallId = String(event.toolCallId ?? id('tool', event.sequence)); let part = message.parts.find((item) => item.type === 'tool-call' && item.toolCallId === toolCallId); if (!part) { part = { type: 'tool-call', toolCallId, name: String(event.name ?? 'Tool'), arguments: event.arguments, status: 'running', startedAt: event.receivedAt }; message.parts.push(part); } break; }
