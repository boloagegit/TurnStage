...
function redactValue(value: string): string { const prefix = value.match(/^\S+\s+/)?.[0] ?? ''; return `${prefix}••••••••`; }
export function redactDeep(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(redactDeep);
  if (!value || typeof value !== 'object') return value;
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, child]) => [key, sensitiveHeaders.has(key.toLowerCase()) || /secret|token|password/i.test(key) ? '••••••••' : redactDeep(child)]));
}
