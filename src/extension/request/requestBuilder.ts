...
import { redactDeep, redactHeaders } from '../security/security';
...
export class RequestBuilder {
  ...
  async build(definition: RequestDefinition, context: ResolutionContext): Promise<PreparedRequest> {
    ...
    const url = String(await resolveTemplate(definition.url, context, this.getSecret));
    let parsed: URL;
    try { parsed = new URL(url); } catch { throw errors.request(localize('Invalid request URL: {url}', { url })); }
    ...
    const headers = await resolveTemplate({ ...definition.headers, ...variant?.headers }, context, this.getSecret) as Record<string, string>;
    const resolvedBody = await resolveTemplate(variant?.body ?? definition.body, context, this.getSecret);
    const body = resolvedBody === undefined ? undefined : JSON.stringify(resolvedBody);
    return { method: definition.method, url, headers, body, timeoutMs: definition.timeoutMs, idleTimeoutMs: definition.idleTimeoutMs, redacted: { method: definition.method, url, headers: redactHeaders(headers), body: redactDeep(resolvedBody), variantId: variant?.id } };
  }
}
