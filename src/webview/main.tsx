...
import { compareTestRuns, nonPassingCases, type TestRunHistoryRecord } from '../shared/testRunHistory';
...
type TestKindFilter = 'contract' | 'adversarial';
const WEBVIEW_STATE_VERSION = 7;
const scrollPositionKeys = ['chat', 'tests.results', 'tests.scenarios', 'adversarial.results', 'adversarial.cases', 'adversarial.campaigns', 'adversarial.timeline', 'events.raw', 'events.normalized', ...SETTINGS_SECTIONS.map((section) => `configure.${section.id}`)] as const;
...
function App(): React.JSX.Element {
  ...
  const [testsSection, setTestsSection] = useState<AutomationSectionId>(AUTOMATION_SECTIONS.includes(savedState?.testsSection as AutomationSectionId) ? savedState!.testsSection as AutomationSectionId : 'scenarios');
  const [selectedTestCases, setSelectedTestCases] = useState<ReadonlyMap<string, TestCaseIdentity>>(() => new Map());
  const [redTeamSection, setRedTeamSection] = useState<RedTeamSectionId>(RED_TEAM_SECTIONS.includes(savedState?.redTeamSection as RedTeamSectionId) ? savedState!.redTeamSection as RedTeamSectionId : 'cases');
  ...
  const [selectedAutomationResultKey, setSelectedAutomationResultKey] = useState(savedState?.selectedAutomationResultKey);
  ...
  useEffect(() => {
    const listener = (event: MessageEvent<HostMessage>) => {
      ...
      if (message.type === 'host.ready') { setLocale(message.locale, message.direction); setLocaleVersion((current) => current + 1); setRemoteName(message.remoteName); setHostKind(message.hostKind ?? 'vscode'); }
        ...
        else if (destination.pane === 'tests') { const section = destination.section === 'campaigns' ? 'scenarios' : destination.section; setRightPaneMode('tests'); setTestsSection(section); focusAfterRender(() => document.getElementById(`automation-${section}`)?.focus()); }
        else if (destination.pane === 'adversarial') { const section = destination.section === 'campaigns' ? 'cases' : destination.section; setRightPaneMode('adversarial'); setRedTeamSection(section); focusAfterRender(() => document.getElementById(`red-team-${section}`)?.focus()); }
      ...
      else if (message.type === 'profile.snapshot') { setProfile(message.profile); setProfileReadOnly(message.readOnly === true); setConnectionResult(undefined); }
      ...
      else if (message.type === 'adversarial.case.deleted') { setLinkedAdversarialCaseEditor(undefined); setNotice(t('Deleted case {name}.', { name: message.scenarioId })); }
      else if (message.type === 'adversarial.case.error') { setLinkedAdversarialCaseEditor({ status: 'error', sourcePath: message.sourcePath, scenarioId: message.scenarioId, message: message.message, conflict: message.conflict }); setNotice(message.message); }
      ...
      else if (message.type === 'contract.case.deleted') { setLinkedContractCaseEditor(undefined); setNotice(t('Deleted case {name}.', { name: message.scenarioId })); }
      else if (message.type === 'contract.case.error') { setLinkedContractCaseEditor({ status: 'error', sourcePath: message.sourcePath, scenarioId: message.scenarioId, message: message.message, conflict: message.conflict }); setNotice(message.message); }
      else if (message.type === 'test.results') { setTestResults(message.results); setAutomationResults(message.automationResults ?? []); }
      else if (message.type === 'test.history') setTestHistory({ profileId: message.profileId, runs: message.runs, baselineRunId: message.baselineRunId });
  ...
  return <main className="app">
    ...
    <section id="main-panel" tabIndex={-1} className="panel" aria-label={t('Test')}>
      <TestWorkspace
        ...
        selectedTestCases={selectedTestCases} setSelectedTestCases={setSelectedTestCases}
        testHistory={testHistory}
        selectedAutomationResultKey={selectedAutomationResultKey} setSelectedAutomationResultKey={setSelectedAutomationResultKey}
        ...
        mappingTestResult={mappingTestResult} remoteName={remoteName} profileDirty={profileDirty} profileReadOnly={profileReadOnly} hostKind={hostKind} diagnostics={diagnostics}
...
function TestWorkspace({ profile, snapshot, runs, networkEntries, testResults, automationResults, contractCaseCatalog, linkedContractCaseEditor, adversarialCaseCatalog, linkedAdversarialCaseEditor, adversarialCaseCollection, setAdversarialCaseCollection, adversarialResultCollection, setAdversarialResultCollection, campaignDashboard, activeEvidenceId, activeTimeline, onCloseEvidence, connectionResult, active, continuationBlocked, draft, setDraft, send, inspectorTab, setInspectorTab, requestPreview, splitPercent, setSplitPercent, splitCustomized, setSplitCustomized, chatViewport, setChatViewport, eventFilters, setEventFilters, collapsedEventTurns, setCollapsedEventTurns, selectedMessageId, selectedRawSequence, selectedNetworkId, acceptedForms, messageActionFeedback, visualFeedback, onMessageActionFeedback, onSelectMessage, onSelectEvent, onCreateMapping, rightPaneMode, setRightPaneMode, testsSection, setTestsSection, selectedTestCases, setSelectedTestCases, testHistory, selectedAutomationResultKey, setSelectedAutomationResultKey, redTeamSection, setRedTeamSection, selectedCampaignId, setSelectedCampaignId, configurationSection, setConfigurationSection, mappingTestResult, remoteName, profileDirty, profileReadOnly, hostKind, diagnostics, scrollPositions, onScrollPositionChange, expandedContractCaseId, setExpandedContractCaseId, expandedAdversarialCaseId, setExpandedAdversarialCaseId, networkInspector, setNetworkInspector }: {
  ...
  selectedTestCases: ReadonlyMap<string, TestCaseIdentity>;
  setSelectedTestCases: React.Dispatch<React.SetStateAction<ReadonlyMap<string, TestCaseIdentity>>>;
  testHistory?: { profileId: string; runs: TestRunHistoryRecord[]; baselineRunId?: string };
  selectedAutomationResultKey?: string;
  setSelectedAutomationResultKey: (key: string | undefined) => void;
  ...
  const activeTestSection: AutomationSectionId = testKind === 'adversarial' ? redTeamSection === 'cases' ? 'scenarios' : 'results' : testsSection;
  const selectTestSection = (section: AutomationSectionId) => {
    if (testKind === 'adversarial') setRedTeamSection(section === 'scenarios' ? 'cases' : 'results');
    else setTestsSection(section);
  };
  const contractSuiteSignature = (profile.tests?.contractSuites ?? []).join('\u001f');
  const adversarialSuiteSignature = (profile.tests?.adversarialSuites ?? []).join('\u001f');
  useEffect(() => {
    if (rightPaneMode !== 'tests' && rightPaneMode !== 'adversarial') return;
    post({ type: testKind === 'contract' ? 'contract.catalog.request' : 'adversarial.catalog.request' });
  }, [rightPaneMode, testKind, profile.id, contractSuiteSignature, adversarialSuiteSignature]);
  useEffect(() => {
    const listener = (event: MessageEvent<HostMessage>) => {
  ...
  const toggleTestCase = (identity: TestCaseIdentity) => {
    ...
    if (!selectedTestCases.has(key) && selectedForProfile.length >= 500) { setSelectionNotice(t('A run can include at most 500 cases. Clear some selected cases before adding more.')); return; }
    setSelectedTestCases((current) => {
      const next = new Map(current);
      if (next.has(key)) next.delete(key);
      else if ([...next.values()].filter((item) => item.profileId === identity.profileId && item.kind === identity.kind).length < 500) next.set(key, identity);
      return next;
    });
  ...
  const selectedRunAvailability = selectedForProfile.reduce((counts, identity) => {
    ...
    const unsupported = Boolean(hostKind !== 'vscode' && identity.kind === 'contract' && scenario && ('faults' in scenario && scenario.faults || 'comparison' in scenario && scenario.comparison || 'performance' in scenario && scenario.performance));
      ...
      <aside className="debug-pane" aria-label={t('Debug and profile configuration')}>
        <header className="right-pane-heading">
          <div className="right-pane-tabs" role="tablist" aria-label={t('Right panel')} onKeyDown={(event) => {
            ...
            <button id="right-pane-tests-tab" type="button" role="tab" tabIndex={rightPaneMode === 'tests' ? 0 : -1} aria-selected={rightPaneMode === 'tests'} aria-controls="right-pane-panel" onClick={() => selectRightPaneMode('tests')}>{t('General tests')}</button>
          ...
          <span>{rightPaneMode === 'debug' ? t('{count} raw events', { count: formatNumber(snapshot?.rawEvents.length ?? 0) }) : rightPaneMode === 'tests' ? t('{count} test cases', { count: formatNumber((profile.tests?.scenarios?.filter((item) => !item.adversarial).length ?? 0) + (contractCaseCatalog?.total ?? 0)) }) : rightPaneMode === 'adversarial' ? t('{count} test cases', { count: formatNumber((profile.tests?.scenarios?.filter((item) => Boolean(item.adversarial)).length ?? 0) + (adversarialCaseCatalog?.total ?? 0)) }) : t('Edits profile JSONC')}</span>
        </header>
        <div id="right-pane-panel" className="right-pane-panel" role="tabpanel" aria-labelledby={`right-pane-${rightPaneMode}-tab`} tabIndex={-1}>
          {rightPaneMode === 'debug'
            ...
            : rightPaneMode === 'tests' || rightPaneMode === 'adversarial'
              ? <div className="unified-test-workspace">
                  <div className="unified-test-workspace__toolbar">
                    <div className="unified-test-workspace__sections" role="tablist" aria-label={t('Test sections')} onKeyDown={(event) => {
                      if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
                      event.preventDefault();
                      const next = event.key === 'Home' || (event.key === 'ArrowLeft' && activeTestSection === 'results') || (event.key === 'ArrowRight' && activeTestSection === 'results') ? 'scenarios' : 'results';
                      selectTestSection(next);
                      focusAfterRender(() => document.getElementById(next === 'scenarios' ? 'unified-test-cases-tab' : 'unified-test-results-tab')?.focus());
                    }}>
                      <button id="unified-test-cases-tab" type="button" role="tab" aria-selected={activeTestSection === 'scenarios'} tabIndex={activeTestSection === 'scenarios' ? 0 : -1} onClick={() => selectTestSection('scenarios')}>{t('Cases')}</button>
                      <button id="unified-test-results-tab" type="button" role="tab" aria-selected={activeTestSection === 'results'} tabIndex={activeTestSection === 'results' ? 0 : -1} onClick={() => selectTestSection('results')}>{t('Results')}</button>
                    </div>
                    {activeTestSection === 'scenarios' && <div className="unified-test-workspace__run-actions"><button type="button" className="primary" disabled={!selectedForProfile.length || testOperation?.state === 'running' || testOperation?.state === 'cancelling'} onClick={() => { if (selectedRunAvailability.unresolved) { setSelectionNotice(t('Some selected cases are no longer available. Refresh the list before running.')); return; } if (selectedRunAvailability.unavailable) { setSelectionNotice(t('Some selected cases need review or are not supported in this app. Open or remove them before running.')); return; } setSelectionNotice(undefined); post({ type: 'test.runSelection', cases: selectedForProfile }); }}>{t('Run selected {count}', { count: formatNumber(selectedForProfile.length) })}</button>{selectedForProfile.length > 0 && <button type="button" onClick={() => { setSelectedTestCases((current) => new Map([...current].filter(([, item]) => item.profileId !== profile.id || item.kind !== testKind))); setSelectionNotice(undefined); }}>{t('Clear selection')}</button>}{completedRunForCurrentKind && <button type="button" onClick={() => selectTestSection('results')}>{t('View test results')}</button>}{selectionNotice && <span role="alert">{selectionNotice}</span>}</div>}
                  </div>
                    ...
                    {activeTestSection === 'results' && <section className="unified-test-history" aria-label={t('Run history')}>
                      ...
                      {!visibleHistoryRuns.length ? <p className="unified-test-history__empty">{t('No recorded runs yet.')}</p> : <>
                        ...
                        {selectedHistoryRun && <div className="unified-test-history__detail">
                          ...
                          <div className="unified-test-history__actions"><span>{baselineHistoryRun?.id === selectedHistoryRun.id ? t('Current baseline') : baselineHistoryRun ? t('Compared with accepted baseline') : t('No baseline selected')}</span><button type="button" disabled={selectedHistoryRun.status !== 'completed' || baselineHistoryRun?.id === selectedHistoryRun.id} onClick={() => confirmHistoryAction({ title: t('Use this run as baseline?'), actionLabel: t('Use as baseline'), detail: t('Future runs will be compared with this run. The previous baseline is not deleted.'), tone: 'default', onConfirm: () => post({ type: 'test.baseline.accept', runId: selectedHistoryRun.id }) })}>{t('Use as baseline')}</button><button type="button" disabled={!nonPassingCases(selectedHistoryRun).filter((item) => item.kind === testKind).length || testOperation?.state === 'running' || testOperation?.state === 'cancelling'} onClick={() => post({ type: 'test.history.rerun', runId: selectedHistoryRun.id, kind: testKind })}>{t('Rerun non-passing cases ({count})', { count: formatNumber(nonPassingCases(selectedHistoryRun).filter((item) => item.kind === testKind).length) })}</button><details className="unified-test-history__export"><summary>{t('Export this run')}</summary><div>{(['json', 'junit', 'html'] as const).map((format) => <button key={format} type="button" disabled={selectedHistoryRun.cases.some((item) => item.evidenceId && !item.evidenceAvailable)} onClick={() => post({ type: 'test.history.export', runId: selectedHistoryRun.id, kind: testKind, format })}>{format === 'junit' ? 'JUnit XML' : format.toUpperCase()}</button>)}</div></details></div>
                          {selectedHistoryRun.sourceRunId && <p className="unified-test-history__source">{t('Rerun of {date}', { date: historyRuns.find((run) => run.id === selectedHistoryRun.sourceRunId)?.startedAt ? formatDateTime(historyRuns.find((run) => run.id === selectedHistoryRun.sourceRunId)!.startedAt) : selectedHistoryRun.sourceRunId })}</p>}
                          <UnifiedTestHistoryCases run={selectedHistoryRun} baseline={baselineHistoryRun} kind={testKind} differences={historyDifferences} outcomeLabel={testRunOutcomeLabel} differenceLabel={testRunDifferenceLabel} onOpenEvidence={(evidenceId) => post({ type: 'test.evidence.open', evidenceId, location: { kind: 'profile', path: 'tests.scenarios' } })} />
                        </div>}
                      </>}
                    </section>}
...
export function adversarialCaseCount(profile: TurnStageProfile, catalog?: AdversarialCaseCatalog): number {
  return (profile.tests?.scenarios?.filter((scenario) => scenario.adversarial).length ?? 0) + (catalog?.total ?? 0);
}

function testRunStatusLabel(status: TestRunHistoryRecord['status']): string {
  return t(status === 'completed' ? 'Completed' : status === 'cancelled' ? 'Cancelled' : 'Failed');
}

function testRunOutcomeLabel(outcome: TestRunHistoryRecord['cases'][number]['outcome']): string {
  if (!outcome) return t('not run');
  const label = { passed: 'Passed', failed: 'Failed', error: 'Error', resisted: 'Resisted', attackSucceeded: 'Attack succeeded', indeterminate: 'Indeterminate', infrastructureError: 'Infrastructure error' } as const;
  return t(label[outcome]);
}

function testRunDifferenceLabel(difference: ReturnType<typeof compareTestRuns>[number]['difference']): string {
  const labels = { 'new-failure': 'New failure', recovered: 'Recovered', 'still-failing': 'Still failing', 'new-case': 'New case', 'removed-case': 'Removed case', 'definition-changed': 'Case changed', 'configuration-changed': 'Settings changed', 'infrastructure-error': 'Execution error', indeterminate: 'Cannot determine', incomplete: 'Incomplete run', unchanged: 'Unchanged' } as const;
...
export function inspectorTabCounts(snapshot: SessionSnapshot | undefined, networkEntries: readonly NetworkExchange[], runs: readonly LocalRunSummary[]): Record<InspectorTab, number> {
  return {
    Network: networkEntries.length,
    'Raw Events': snapshot?.rawEvents.length ?? 0,
    Normalized: snapshot?.normalizedEvents.length ?? 0,
    Metrics: 0,
...
export function Inspector({ profile, snapshot, runs = [], networkEntries = [], active = false, tab, setTab, requestPreview, full = false, interactive = true, eventFilters = normalizeInspectorEventFilters(), onEventFiltersChange = () => undefined, collapsedEventTurns = normalizeCollapsedEventTurns(), onCollapsedEventTurnsChange = () => undefined, onCreateMapping, selectedSequence, selectedNetworkId, onSelectEvent, scrollPositions = {}, onScrollPositionChange = () => undefined, networkInspector, onNetworkInspectorChange }: { profile?: TurnStageProfile; snapshot?: SessionSnapshot; runs?: LocalRunSummary[]; networkEntries?: NetworkExchange[]; active?: boolean; tab: InspectorTab; setTab: (tab: InspectorTab) => void; requestPreview: unknown; full?: boolean; interactive?: boolean; eventFilters?: InspectorEventFilters; onEventFiltersChange?: (filters: InspectorEventFilters) => void; collapsedEventTurns?: CollapsedEventTurns; onCollapsedEventTurnsChange?: (turns: CollapsedEventTurns) => void; onCreateMapping?: (event: RawStreamEvent) => void; selectedSequence?: number; selectedNetworkId?: string; onSelectEvent?: (event?: Record<string, unknown>) => void; scrollPositions?: Partial<Record<ScrollPositionKey, number>>; onScrollPositionChange?: (key: ScrollPositionKey, value: number) => void; networkInspector?: NetworkInspectorState; onNetworkInspectorChange?: (state: NetworkInspectorState) => void }): React.JSX.Element {
  ...
  const tabCounts = inspectorTabCounts(snapshot, networkEntries, runs);
  ...
  return <div className={`inspector-content ${full ? 'full' : ''}`}>
    <div className="mini-tabs" role="tablist" aria-orientation="horizontal" aria-label={t('Evidence views')}>{availableTabs.map((item) => { const count = tabCounts[item]; const label = t(inspectorTabLabel(item)); return <button key={item} id={inspectorTabId(item)} role="tab" aria-label={count ? `${label} (${formatNumber(count)})` : label} aria-selected={effectiveTab === item} aria-controls={inspectorPanelId(item)} tabIndex={effectiveTab === item ? 0 : -1} disabled={!interactive} onClick={() => setTab(item)} onKeyDown={(event) => handleTabKeyDown(event, item)}>{label}{count ? <span className={`inspector-tab-count${item === 'Errors' ? ' inspector-tab-count--error' : ''}`} aria-hidden="true">{count > 99 ? '99+' : formatNumber(count)}</span> : null}</button>; })}</div>
...
export function normalizeWebviewState(value: unknown): WebviewState | undefined {
  ...
  const savedTestsSection = (value as { testsSection?: unknown }).testsSection;
  const savedCombinedRedTeam = (value as { rightPaneMode?: unknown }).rightPaneMode === 'tests' && candidate.testKind === 'adversarial';
  const normalizedPane = savedCombinedRedTeam ? 'adversarial' : isRightPaneMode(candidate.rightPaneMode) ? candidate.rightPaneMode : undefined;
  ...
  return {
    ...
    ...boundedOptionalString('selectedAutomationResultKey', candidate.selectedAutomationResultKey),
...
function boundedOptionalString<K extends 'sessionId' | 'selectedMessageId' | 'selectedNetworkId' | 'activeEvidenceId' | 'selectedAutomationResultKey' | 'expandedContractCaseId' | 'expandedAdversarialCaseId' | 'selectedCampaignId'>(key: K, value: unknown): Partial<Record<K, string>> {
