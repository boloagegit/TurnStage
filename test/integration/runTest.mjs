...
async function runCliSmoke(workspace) {
  ...
  if (parsed?.summary?.attackSucceeded !== 1 || parsed?.summary?.resisted !== 1 || parsed?.summary?.passed !== 2) throw new Error(`Headless CLI did not execute the expected linked JSONC, adversarial CSV, and inline contract mock cases: ${result.stdout}`);
...
async function createWorkspace(trustMode, mockPort) {
  ...
  await writeFile(path.join(workspace, '.vscode', 'turnstage', 'profiles', 'integration.turnstage.jsonc'), JSON.stringify({ version: 1, id: 'integration', name: 'Integration Profile', environment: 'local', opening: { mode: 'request', request: { method: 'POST', url: '${env.baseUrl}/agent/opening', body: {} } }, conversation: { send: { method: 'POST', url: '${env.baseUrl}/basic/chat/stream', variants: [{ id: 'first', body: { message: { $value: 'input.text' } } }] } }, stream: { transport: 'sse', mappings: [{ id: 'message', match: { event: 'message' }, emit: { type: 'content.text.delta', text: { path: '$.text' } } }, { id: 'done', match: { event: 'done' }, emit: { type: 'stream.completed' } }] }, tests: { contractSuites: ['.vscode/turnstage/tests/integration.tests.jsonc'], adversarialSuites: ['.vscode/turnstage/tests/integration.adversarial.jsonc', '.vscode/turnstage/tests/integration.adversarial.csv'], qualityRubrics: [{ id: 'integration-quality', name: 'Integration quality', criteria: [{ id: 'relevance', label: 'Relevance', description: 'The disclosed response addresses the fixed integration request.' }] }], reporting: { formats: ['json', 'junit'], outputDirectory: '.turnstage/reports' }, scenarios: [{ id: 'integration-contract', name: 'Integration contract', comparison: { baseline: { label: 'Integration baseline', environment: 'local' }, candidate: { label: 'Integration candidate', environment: 'local' } }, performance: { thresholds: { 'scenario.durationMs': 5000 }, regression: { 'scenario.durationMs': { maxIncreaseMs: 2000, maxIncreasePercent: 1000 } } }, steps: [{ id: 'first-message', input: 'Hello from Test Explorer', assertions: [{ path: 'turn.state', operator: 'equals', value: 'completed' }] }] }] } }, null, 2));
  await writeFile(path.join(workspace, '.vscode', 'turnstage', 'tests', 'integration.tests.jsonc'), JSON.stringify({ format: 'turnstage-contract-suite', version: 1, id: 'integration-linked-contracts', name: 'Integration linked contracts', sourceBinding: { sourceGlobs: ['src/contracts/**'], components: ['chat'] }, cases: [{ id: 'integration-linked-contract', name: 'Integration linked contract', tags: ['integration', 'linked-contract'], steps: [{ id: 'linked-first-message', input: 'Hello from linked Test Explorer suite', assertions: [{ path: 'turn.state', operator: 'equals', value: 'completed' }] }] }] }, null, 2));
  ...
  const csvColumns = ['case_id', 'case_name', 'description', 'tags', 'enabled', 'turn_index', 'turn_id', 'turn_name', 'user_message', 'forbidden_content_json', 'forbid_urls', 'forbid_ctas', 'forbid_tools', 'forbidden_events_json', 'additional_forbidden_content_json', 'additional_forbid_urls', 'additional_forbid_ctas', 'additional_forbid_tools', 'additional_forbidden_events_json', 'max_turns', 'timeout_ms', 'stop_on_attack_succeeded', 'repetitions', 'fail_fast'];
  const csvCase = ['integration-csv-resistance', 'Integration CSV resistance', '', JSON.stringify(['integration', 'csv']), 'true', '1', 'probe', '', 'CSV linked direct probe', JSON.stringify(['blocked-marker']), 'false', 'false', 'false', '[]', '[]', 'false', 'false', 'false', '[]', '1', '10000', 'true', '2', 'false'];
  await writeFile(path.join(workspace, '.vscode', 'turnstage', 'tests', 'integration.adversarial.csv'), `\uFEFF${csvRow(csvColumns)}\r\n${csvRow(csvCase)}\r\n`);
