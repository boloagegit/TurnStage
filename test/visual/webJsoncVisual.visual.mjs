...
try {
  ...
  await page.goto(url);
  await page.locator('.profile-library').waitFor().catch(async (error) => {
    console.error(JSON.stringify({ pageErrors: errors, body: (await page.locator('body').innerText()).slice(0, 1000) }, null, 2));
    throw error;
  });
  ...
  const editorSource = () => page.locator('.jsonc-editor-layout').getAttribute('data-source');
  const editorLayout = async () => page.locator('.profile-reference-editor').evaluate((container) => {
    const outer = container.getBoundingClientRect();
    const input = container.querySelector('.cm-editor').getBoundingClientRect();
  ...
  assert.ok(layout.outerHeight > 400 && layout.inputHeight >= layout.outerHeight - 60 && layout.topGap <= 2 && layout.bottomGap <= 2, `JSONC editor fills the dialog below its toolbar: ${JSON.stringify(layout)}`);
  await page.getByRole('navigation', { name: 'Sections' }).getByRole('button', { name: /opening/u }).waitFor();
  assert.equal(await page.locator('.profile-reference-editor textarea').count(), 0, 'Edit mode uses one CodeMirror text layer instead of an overlaid textarea');
  await page.getByRole('navigation', { name: 'Sections' }).getByRole('button', { name: /opening/u }).click();
  const sectionJump = await editor.evaluate((element) => ({ scroll: element.closest('.cm-scroller')?.scrollTop ?? 0, selection: globalThis.getSelection()?.anchorOffset ?? 0 }));
  assert.ok(sectionJump.scroll >= 0, `Section jump keeps the CodeMirror editor active: ${JSON.stringify(sectionJump)}`);
  ...
  const initial = await editorSource();
  assert.match(initial, /basic-sse-chat-copy/u);
  await editor.focus();
  await editor.press('End');
  await editor.type('x');
  assert.notEqual(await editorSource(), initial, 'Typing edits the CodeMirror JSONC source directly');
  await editor.fill('{"version":1}');
  await page.getByRole('button', { name: 'Format' }).click();
  assert.match(await editorSource(), /\n {2}"version": 1\n/u, 'Format applies stable two-space JSONC formatting');
  await editor.fill(initial);
  await editor.fill('{ broken');
  await page.getByRole('button', { name: 'Save', exact: true }).click();
  await page.getByRole('alert').waitFor();
  layout = await editorLayout();
  assert.ok(layout.inputHeight > 300 && layout.inputHeight < layout.outerHeight - 20, `Validation notice leaves room to edit: ${JSON.stringify(layout)}`);
  await screenshot(page, 'jsonc-validation');
  assert.equal(await editorSource(), '{ broken', 'Invalid draft remains editable');
  ...
  assert.match(await page.locator('.jsonc-editor-layout').getAttribute('data-source'), /Preserved browser-local comment/u, 'Browser-local source opens directly for editing after reload');
  const unsaved = `${valid}\n// Recovered draft\n`;
  await page.getByRole('textbox', { name: 'JSONC' }).fill(unsaved);
  await page.waitForFunction(() => JSON.parse(localStorage.getItem('turnstage.web.profileDrafts.v1') ?? '{}')['basic-sse-chat-copy']?.draft.includes('Recovered draft'));
  await page.reload();
  await page.getByRole('button', { name: 'Profile actions: JSONC Edited' }).click();
  await page.getByRole('menuitem', { name: 'Edit JSONC' }).click();
  assert.equal(await page.locator('.jsonc-editor-layout').getAttribute('data-source'), unsaved, 'Unsaved JSONC draft is restored after reload');
  await page.getByRole('textbox', { name: 'JSONC' }).fill(valid);
  await page.waitForFunction(() => !JSON.parse(localStorage.getItem('turnstage.web.profileDrafts.v1') ?? '{}')['basic-sse-chat-copy']);
  await page.getByRole('button', { name: 'Close' }).click();
